...
Assets use an application-owned `AssetServer`, typed `Handle<T>`, registered importers/codecs, and dependency-aware reload. The world example uses source-relative glTF/HDR paths and stores cooked data directly under `asset/manifests` and `asset/blobs`, with `asset/writer.lock` coordinating writes. Container headers retain format version checks. Missing or stale assets are rebuilt from `content`. `ZENITH_CONTENT` and `ZENITH_ASSET_CACHE` override its directories. See the [asset API, extension examples, and packaging guide](zenith-asset/README.md). Renderer flags use `DebugMode::DIFFUSE_SH` and `DebugMode::empty()`.
