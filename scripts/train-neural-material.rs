...
const LAYERS: [(usize, usize, usize, usize); 3] =
    [(16, 32, 0, 512), (32, 32, 544, 1568), (32, 4, 1600, 2112)];
...
fn main() -> std::io::Result<()> {
    ...
    for &(inputs, outputs, matrix, _) in &LAYERS {
        for w in &mut weights[matrix..matrix + inputs * outputs] {
            *w = (random(&mut rng) * 2.0 - 1.0) * (6.0 / inputs as f32).sqrt();
        }
    }
    const STEPS: usize = 120000;
    ...
    for step in 1..=STEPS {
        ...
        for _ in 0..BATCH {
            ...
            for (layer, &(inputs, outputs, matrix, bias)) in LAYERS.iter().enumerate() {
                for row in 0..outputs {
                    ...
                    for col in 0..inputs {
                        value += weights[matrix + row * inputs + col] * activations[layer][col];
                    }
            ...
            for i in 0..4 {
                let error = activations[3][i] - target[i];
                delta[i] = error * (0.5 / BATCH as f32);
                loss += error * error;
            }
        ...
        if step % 10000 == 0 {
            println!(
                "step {step}: RMSE {:.6}",
                (loss / (BATCH * 4) as f32).sqrt()
            );
        }
    }
    let destination = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "content/neural_material.bin".into());
