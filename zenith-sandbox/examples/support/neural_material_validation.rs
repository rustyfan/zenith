...
pub fn run() -> Result<()> {
    ...
    ensure!(instance.validation_enabled(), "neural material validation requires Vulkan validation layers");
    let gpu = Gpu::new(instance.clone(), std::env::var("ZENITH_ADAPTER").ok().as_deref())?;
    ...
    writeln!(report, "backend,width,height,cpu_rmse,reference_rmse,max_cpu_error,gpu_ms")?;
    let mut cache = ResourceCache::default();
    for backend in backends {
        renderer.settings.backend = backend;
        for (width, height) in [(1u32, 1u32), (19, 7), (256, 256)] {
            let readback = gpu.allocate(width as u64 * height as u64 * 16, MemoryDomain::Readback)?;
            ...
            graph.pass("readback", vec![output.read(Access::COPY_READ), destination.write(Access::COPY_WRITE)], move |ctx| {
                ctx.commands.copy(&ctx.buffer(output)?, &ctx.buffer(destination)?)
            })?;
            graph.pass("host", vec![destination.read(Access::HOST_READ)], |_| Ok(()))?;
            let (commands, timings) = graph.record_profiled(true)?;
            commands.submit()?.wait(10_000_000_000)?;
            let milliseconds = timings.unwrap().read()?.into_iter().find(|(name, _)| name.starts_with("neural_material_")).unwrap().1;
            ...
            for (index, pixel) in bytes.chunks_exact(16).enumerate() {
                let uv = [((index as u32 % width) as f32 + 0.5) / width as f32, ((index as u32 / width) as f32 + 0.5) / height as f32];
                let expected = renderer.evaluate(uv);
                let reference = NeuralMaterialRenderer::reference(uv);
                for channel in 0..4 {
                    let actual = f32::from_le_bytes(pixel[channel * 4..channel * 4 + 4].try_into()?);
                    ensure!(actual.is_finite() && (0.0..=1.0).contains(&actual), "invalid output at {index}:{channel}");
            ...
            println!("{} {width}x{height}: CPU RMSE {cpu_rmse:.6}, max {maximum:.6}, reference RMSE {reference_rmse:.6}, {milliseconds:.4} ms", backend.label());
            writeln!(report, "{backend:?},{width},{height},{cpu_rmse},{reference_rmse},{maximum},{milliseconds}")?;
            ensure!(cpu_rmse < 0.003 && maximum < 0.025, "{} differs from CPU reference", backend.label());
            ensure!(reference_rmse < 0.035, "material reconstruction quality regressed");
        }
        ...
        let destination = graph.import_buffer(readback.clone());
        graph.pass("capture", vec![image.read(Access::COPY_READ), destination.write(Access::COPY_WRITE)], move |ctx| {
            let texture = ctx.image(image)?;
            let buffer = ctx.buffer(destination)?;
            unsafe { ctx.commands.read_image(&texture, &buffer, &[vk::BufferImageCopy::default()
                .image_subresource(vk::ImageSubresourceLayers::default().aspect_mask(vk::ImageAspectFlags::COLOR).layer_count(1))
                .image_extent(vk::Extent3D { width, height, depth: 1 })]) }
        })?;
        graph.pass("capture_host", vec![destination.read(Access::HOST_READ)], |_| Ok(()))?;
        graph.record()?.submit()?.wait(10_000_000_000)?;
        let mut pixels = vec![0; (width * height * 4) as usize];
        readback.read(0, &mut pixels)?;
        save_bmp(&directory.join(format!("{backend:?}.bmp")), width, height, pixels)?;
    }
    drop(renderer);
    drop(cache);
    drop(descriptors);
    drop(gpu);
    ensure!(instance.validation_errors().is_empty(), "Vulkan validation errors: {:?}", instance.validation_errors());
    println!("Neural material validation passed. Captures: {}", directory.display());
    Ok(())
}
