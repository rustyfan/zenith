...
pub fn features(uv: [f32; 2]) -> [f32; 16] {
    ...
    let phases = [u, v, u + v, u - v, 2.0 * u, 2.0 * v, 2.0 * u + v, u + 2.0 * v];
    std::array::from_fn(|i| if i % 2 == 0 { phases[i / 2].sin() } else { phases[i / 2].cos() })
}
...
pub fn evaluate(weights: &[f32], uv: [f32; 2]) -> [f32; 4] {
    ...
    for (layer, &(inputs, outputs, matrix, bias)) in LAYERS.iter().enumerate() {
        ...
        for row in 0..outputs {
            ...
            for column in 0..inputs { value += weights[matrix + row * inputs + column] * activation[column]; }
...
pub fn decode(bytes: &[u8]) -> Result<Vec<f32>> {
    ensure!(bytes.len() == 8 + PARAMETER_COUNT * 4 && &bytes[..4] == b"ZNM1", "invalid neural material model");
    ensure!(u32::from_le_bytes(bytes[4..8].try_into()?) == PARAMETER_COUNT as u32, "invalid neural material parameter count");
    let weights: Vec<_> = bytes[8..].chunks_exact(4).map(|b| f32::from_le_bytes(b.try_into().unwrap())).collect();
    ensure!(weights.iter().all(|w| w.is_finite() && w.abs() < 65504.0), "invalid neural material weights");
    Ok(weights)
}
