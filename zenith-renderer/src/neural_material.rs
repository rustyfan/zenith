...
const MATRIX_SHARED_BYTES: u32 = 16 * 32 * (size_of::<u16>() + size_of::<f32>()) as u32;
...
impl NeuralMaterialRenderer {
    pub fn new(gpu: &Arc<Gpu>) -> Result<Self> {
        ...
        if caps.vector_f16(vk::ShaderStageFlags::COMPUTE, 32) {
            for &(inputs, outputs, matrix, _) in &model::LAYERS {
                packed.push(gpu.cooperative_vector_weights(
                    &half[matrix..matrix + inputs * outputs],
                    outputs as u32,
                    inputs as u32,
                )?);
            }
            variants.push((
                ...
                vec!["spvCooperativeVectorNV"],
            ));
        }
        if caps.matrix_f16(16, 16, 16) && caps.max_shared_memory >= MATRIX_SHARED_BYTES {
            variants.push((
                NeuralBackend::CooperativeMatrix,
                vec!["spvCooperativeMatrixKHR"],
            ));
        }
        if caps.max_shared_memory.saturating_sub(
            caps.matrix2_properties
                .cooperative_matrix_workgroup_scope_reserved_shared_memory,
        ) >= MATRIX_SHARED_BYTES
            && [(16, 32, 16), (16, 32, 32), (16, 16, 32)]
                .iter()
                .all(|&(m, n, k)| caps.matrix2_f16(m, n, k, 32))
        {
            variants.push((
                NeuralBackend::CooperativeMatrix2,
                vec!["spvCooperativeMatrixKHR", "SPV_NV_cooperative_matrix2"],
            ));
        }
        let mut kernels = Vec::new();
        for (backend, capabilities) in variants {
            let group = if backend == NeuralBackend::CooperativeMatrix {
                caps.subgroup_size
            } else {
                32
            };
            ...
            let shader = gpu.compile_shader_with_options(
                "content/shaders/neural_material_inference.slang",
                "main",
                ShaderStage::Compute,
                ShaderOptions {
                    capabilities: &capabilities,
                    defines: &[("BACKEND", &backend_define), ("GROUP_SIZE", &group_define)],
                },
            )?;
            kernels.push((backend, gpu.compute(&shader)?));
        }
        let backend = if kernels
            .iter()
            .any(|(b, _)| *b == NeuralBackend::CooperativeVector)
        {
            NeuralBackend::CooperativeVector
        } else {
            kernels.last().unwrap().0
        };
        let [vertex, fragment] = gpu.compile_shaders([
            ...
            (
                "content/shaders/neural_material.slang",
                "main",
        ...
        Ok(Self {
            ...
            fragment,
            pipeline: None,
            settings: NeuralMaterialSettings {
                backend,
                rotation: 0.0,
                exposure: 1.2,
                error_view: false,
    ...
    pub fn evaluate(&self, uv: [f32; 2]) -> [f32; 4] {
        model::evaluate(&self.weights, uv)
    }
    ...
    pub fn reconstruct(
        ...
        let count = width
            .checked_mul(height)
            .filter(|n| *n > 0 && *n <= 65535 * 16)
            .ok_or_else(|| {
                anyhow::anyhow!("neural material extent must contain 1 to 1048560 texels")
            })?;
        ...
        let pipeline = self
            ...
            .iter()
            .find(|(b, _)| *b == backend)
            .ok_or_else(|| anyhow::anyhow!("{} unavailable", backend.label()))?
            .1
            .clone();
        ...
        let half = builder.import_buffer(
            self.half_weights
                .as_ref()
        ...
        for (slot, memory) in packed.iter_mut().zip(&self.packed) {
            *slot = builder.import_buffer(memory.clone());
        }
        let mut uses = vec![
            output.write(Access::COMPUTE_WRITE),
            canonical.read(Access::COMPUTE_READ),
            half.read(Access::COMPUTE_READ),
        ];
        if backend == NeuralBackend::CooperativeVector {
            uses.extend(packed.iter().map(|id| id.read(Access::COMPUTE_READ)));
        }
        builder.pass(
            format!("neural_material_{}", backend as u32),
            uses,
            move |ctx| {
                ...
                if backend == NeuralBackend::CooperativeVector {
                    for (address, id) in addresses.iter_mut().zip(packed) {
                        let memory = ctx.buffer(id)?;
                        *address = memory.address();
                        reachable.push(memory);
                    }
                }
                let root = ctx.arguments(&InferenceRoot {
                    weights: canonical.address(),
                    half_weights: half.address(),
                    packed: addresses,
                ...
                let samples = if matches!(
                    backend,
                    NeuralBackend::CooperativeMatrix | NeuralBackend::CooperativeMatrix2
                ) {
                    16
                } else {
                    64
    ...
    pub fn render(&mut self, builder: &mut RenderGraphBuilder<'_>, output: ImageId) -> Result<()> {
            ...
            self.settings.rotation.is_finite()
                && self.settings.exposure.is_finite()
                && self.settings.exposure >= 0.0,
            "invalid neural material display settings"
        );
        ...
        builder.pass(
            "neural_material_display",
            vec![material.read(FRAGMENT_READ), output.write(COLOR_WRITE)],
            move |ctx| {
                let material = ctx.buffer(material)?;
                let root = ctx.arguments(&DisplayRoot {
                    ...
                    error_view: settings.error_view.into(),
                    flat_view: settings.flat_view.into(),
                ...
                unsafe {
                    ctx.commands
                        .draw(&pipeline, &root, &root, 0..3, 0..1, &[material])?;
                }
